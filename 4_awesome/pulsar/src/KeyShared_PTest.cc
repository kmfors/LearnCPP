...
private:
    void config(){
        ...
        // 基于密钥的批处理,用于Key_shared设置
        _config.setBatchingType(ProducerConfiguration::BatchingType::KeyBasedBatching);   
    }
...
public:
    ...
    void Send(KeyValue& kvMsg) {
         Message msg = MessageBuilder().setContent(kvMsg).build();
         _worker.sendAsync(msg, [&kvMsg](Result code, const MessageId& msgId){
            std::cout << "Received ack for msg: [" << kvMsg.getValueAsString() 
            << ":" << "] with code: "
            << code << " -- MsgId: " << msgId << std::endl;
         });
